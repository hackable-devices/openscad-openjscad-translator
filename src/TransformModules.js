define("TransformModules", ["Globals", "Context"], function(Globals, Context){
    ...
    function ProjectionTransform(a) {
        TransformModule.call(this, a);
    };


    ProjectionTransform.prototype.evaluate = function(parentContext, inst) {
        inst.argvalues = [];
        ...
        var context = Context.newContext(parentContext, ["cut"], [], inst);

        var cut = Context.contextVariableLookup(context, "cut", false);

        return this.transformChildren(inst.children, context, function(){
            return _.template('.projection(<%=v%>)', {v:cut});
        });
    };
    ...
    ColorTransform.prototype.evaluate = function(parentContext, inst){
        ...
        return this.transformChildren(inst.children, context, function(){
            return _.template('.setColor(<%=r%>, <%=g%>, <%=b%>, 255)', {r:v[0], g:v[1], b:v[2]});
        });
    ...
    RotateExtrudeTransform.prototype.evaluate = function(parentContext, inst){
        ...
        var context = Context.newContext(parentContext, ["convexity", "$fn", "$fs", "%fa"], [], inst);

        var convexity = Context.contextVariableLookup(context, "convexity", 100);
        var fnd = Context.contextVariableLookup(context, "$fn", Globals.FN_DEFAULT);
        var fs = Context.contextVariableLookup(context, "$fs", Globals.FS_DEFAULT);
        var fa = Context.contextVariableLookup(context, "$fa", Globals.FA_DEFAULT);
        
        return this.transformChildren(inst.children, context, function() {
            var template = _.template(".rotate_extrude({convexity: <%=conv%>, fn: <%=fn%>})", {conv:convexity, fn:fnd});
